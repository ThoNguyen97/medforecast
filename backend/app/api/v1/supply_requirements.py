...
async def get_supply_requirements_summary(
    disease_type: Optional[str] = Query(None, description="Không còn dùng — engine DSS hợp nhất không tách vật tư theo bệnh; giữ tham số để tương thích API cũ."),
    ...
    """Tóm tắt vật tư thiếu hụt — đọc từ CÙNG tầng Cảnh báo thiếu hụt
    (dss_dashboard.tang1_tang2 → dss_alerts.alert_rows) thay vì SupplyRequirement
    (bảng đã dừng sinh dữ liệu sau khi bỏ generate_requirements_for_forecast —
    xem _archive/README.md), để trang Báo cáo và trang Cảnh báo không bao giờ
    lệch số với nhau.

    Mỗi dòng: tổng nhu cầu dự báo `horizon_days` ngày tới, tồn khả dụng hiện
    tại (FEFO), và mức thiếu = max(0, nhu cầu − tồn khả dụng).
    """
