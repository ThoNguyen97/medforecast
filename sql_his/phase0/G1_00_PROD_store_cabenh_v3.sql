/* =============================================================================
   G1 — THỦ TỤC CA BỆNH, BẢN 3 (thay thế 03_PROD_store_tong_hop.sql)
   =============================================================================
   ...
   C. BA BẢNG ĐẦU RA MỚI (xem Phase0_01 để biết cấu trúc)
      ...
      (Tiêu hao TOÀN VIỆN đã chuyển sang thủ tục riêng ở G1:
       usp_MedForecast_DayTieuHaoToanVien — khác vũ trụ dữ liệu, khác cửa sổ
       thời gian, khác nhịp chạy. Thủ tục này KHÔNG còn ghi MF_TieuHao_Tong.)
...
BEGIN
    ...
    DECLARE @NhomChuan NVARCHAR(500),
            @nDongPhanCap INT = 0, @nDongTieuHao INT = 0,
            @nONhoPhanCap INT = 0,
    ...
    BEGIN TRY
        ...
        /* ★ Hai bảng phân cấp — cùng kiểu xoá-rồi-chèn, cùng phạm vi nhóm.
           (Bảng thứ ba, MF_TieuHao_Tong, đã tách sang thủ tục riêng ở G1 —
           xem G1_02_PROD_store_tieuhao_toanvien.sql.)                        */
...
/* =============================================================================
   ...
   nhóm J09-J18. Tiêu hao toàn viện không bị ảnh hưởng vì đã nằm ở thủ tục
   riêng.
